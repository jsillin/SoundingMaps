...
for i in range(0,120):
    ...
    In this version, I also plot CAPE and CIN shadings and surface-based parcel
    paths for sounding locations with CAPE >100J/kg. Change this value using
    the capemin variable to plot more or fewer parcel paths.
    ...
    capemin=100 #set the minimum value of CAPE for which CAPE/CIN/parcel paths plot
    ...
    leg = ax1.legend(handles=[dashed_red_line,dashed_green_line,dashed_purple_line,pink_line,red,blue],title='Sounding Legend',loc=4,framealpha=1)
    leg.set_zorder(100)

    #######SAVE FIGURE ###########
    plt.savefig(output_dir+'/GFS/gfs_hrly_pytpesound_cape_'+dtfs+'.png',bbox_inches='tight',pad_inches=0.1)
    fcst_hr = str(0)
    plt.close()
    plt.clf()
