...
for i in range(0,120):
    ...
    ax1.set_extent((255, 270, 29, 41))#, crs = zH5_crs)    # Set a title and show the plot
    ...
    Step 1: decide on bounds for your area of interest. You have 15 degrees of
    longitude and 12 degrees of latitude to work with. Changing these numbers
    will require re-calibrating the plot locations! In set_extent above, put
    your bounds in.

    Step 2: change startlon below to be your eastern bound + 1.6 degrees. Leave
    ...
    sound_lons = []
    sound_lats = []
    lat_delts = [.6,2.5,4.5,6.4,8.4,10.25]
    r=5
    for i in range(0,r):
    ...
    ######## Save the plot
    plt.savefig(output_dir+'/GFS/gfs_hrly_pytpe_reg_sound_'+dtfs+'.png',bbox_inches='tight',pad_inches=0.1)
    fcst_hr = str(0)
    plt.close()
    plt.clf()
